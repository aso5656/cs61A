...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # Phase 1: Add an entrance to the ehxit
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        if exit:
            # for previous place, change its entrance to this instance
            exit.entrance = self
        # END Problem 2
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            ants = [self,place.ant]
            if ants[0].can_contain(ants[1]) or ants[1].can_contain(ants[0]):
                for i in [0,1]:
                    ant = ants[i]
                    the_other_ant = ants[1-i]
                    if ant.can_contain(the_other_ant):
                        ant.contain_ant(the_other_ant)
                        #container ant takes the place
                        place.ant = ant
                        break
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        # current place
        rng, place = 0, self.place
        # place.bees=[],check this place any bees, if not then next place(place.entrance)
        while rng <= self.max_range:
            if place is beehive:
                return None
            else:
                if rANTdom_else_none(place.bees) and rng >= self.min_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        To damage the FireAnt, call the reduce_armor method inherited from Ant. 
        Do not call self.reduce_armor, or you'll end up stuck in a recursive loop
        (Can you see why? That would be calling self.reduce_armor inside self.reduce_armor)
        """
        # BEGIN Problem 5
        ...
        if bees:
            for bee in bees:
                ...
                if not self.place:
                    if bee.place:  # check if bee removed before additional damage
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if 0 < self.digesting <= self.time_to_digest:
            self.digesting += 1
        else:
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        # add insect regardless of is_watersafe
        Place.add_insect(self, insect)
        if not insect.is_watersafe:
            # remove insect if is_watersafe False
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        # True queen
            ...
            # attack using parent class action
            ...
            while place_behind:
                ...
                # only one ant at one place
                if ant:
                    # if the ant been buffed, set an attr 'buffed' on it
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        return self.contained_ant is None and isinstance(other,ContainerAnt) == False
        # END Problem Optional 2
...
def make_slow(action, bee):
    ...
    returns a new action method that 
    performs the original action on every turn that gamestate.time is even
    """
...
def apply_status(status, bee, length):
    ...
    def new_action_with_counter(gamestate):
        nonlocal length
        print("DEBUG:",length)
        if length <= 0:
    ...
    bee.action = new_action_with_counter
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        if target is None or hasattr(self,'forward'):
            return 
        else:
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        damage = LaserAnt.damage - 0.2*distance - 0.05*self.insects_shot
        print("DEBUG:",damage)
