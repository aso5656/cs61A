...
def balanced(m):
    ...
    if is_planet(m):
        ...
        if balanced(l_end) and balanced(r_end):
            return length(l)*total_weight(l_end) == length(r)*total_weight(r_end)
        else:
            return False
        
    # l,r = left(m),right(m)
    # l_end,r_end = end(l),end(r)
    # torque = length(l)*total_weight(l_end) == length(r)*total_weight(r_end)
    ...
    # elif is_mobile(l_end) or is_mobile(r_end):
    #     mobile_to_check = l_end if is_mobile(l_end) else r_end
    #     if balanced(mobile_to_check):
    #         return torque
    #     else:
    #         return False
...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    l,u = -lower_bound(y),-upper_bound(y)
    if l >u:
        l,u = u,l
    opposite_y = interval(l,u)
