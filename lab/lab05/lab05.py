...
def sprout_leaves(t, leaves):
    ...
    """
    "*** YOUR CODE HERE ***"
    leaves_brach = [tree(leaf) for leaf in leaves]
    if is_leaf(t):
...
def add_trees(t1, t2):
    ...
    #if any of t1,t2 is a leaf, return a tree adding the label
    if is_leaf(t1) or is_leaf(t2):
        branch = branches(t2) if is_leaf(t1) else branches(t1)
        return tree(label_sum,branch)
        ...
        if len_diff>0:
            bc_1 = branches(t1) + len_diff*[tree(0)]
            
        elif len_diff<0:
            
            bc_2 = branches(t2) + (-len_diff)*[tree(0)]
            
        
        return tree(label_sum,[add_trees(b1,b2) for b1,b2 in zip(bc_1,bc_2)])
